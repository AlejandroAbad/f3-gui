...
function generarConversorDesdeMaestro(maestro) {
	...
	return (idElemento) => {
		if (Array.isArray(maestro?.datos)) {
			...
			if (elemento) return elemento.nombre;
		}
		return idElemento
	}
}
...
function textoDeFiltro(filtros, clave, Maestros) {
	...
	switch (clave) {
		...
		case 'estado': return <>Estado del pedido {textoDeModo(modo, valor)} {valoresATexto(valor, generarConversorDesdeMaestro(Maestros.maestroEstados))}</>
		...
		case 'pedido.codigoAlmacenServicio': return <>Almacén de servicio {textoDeModo(modo, valor)} {valoresATexto(valor, generarConversorDesdeMaestro(Maestros.maestroAlmacenes))}</>
