...
function generarConversorDesdeMaestro(maestro) {

	return (idElemento) => {
		if (Array.isArray(maestro?.datos)) {
			let elemento = maestro.datos.find(e => e.id === idElemento || e.codigo === idElemento);
			if (elemento) {
				if (elemento.ambito) {
					return `${elemento.ambito} - ${elemento.nombre}`
				}
...
function textoDeFiltro(filtros, clave, Maestros) {
	...
	switch (clave) {
		case 'estado': return <>Estado de la transmisión {textoDeModo(modo, valor)} {valoresATexto(valor, generarConversorDesdeMaestro(Maestros.maestroEstados))}</>
		case 'tipo': return <>Tipo de transmisión {textoDeModo(modo, valor)} {valoresATexto(valor, generarConversorDesdeMaestro(Maestros.maestroTipos))}</>
		case 'conexion.metadatos.ip': return <>IP de origen {textoDeModo(modo, valor)} {valoresATexto(valor)}</>
		case 'conexion.metadatos.programa': return <>Programa de farmacia {textoDeModo(modo, valor)} {valoresATexto(valor, generarConversorDesdeMaestro(Maestros.maestroProgramas))}</>
		case 'conexion.metadatos.autenticacion.usuarioNormalizado': return <>Solicitante {textoDeModo(modo, valor)} {valoresATexto(valor)}</>
		case 'fechaCreacion': return <>Transmisiones recibidas entre el <ChipValor valor={format(filtro['$gte'], 'dd-MM-yyyy HH:mm:ss', { locale: es })} /> y el <ChipValor valor={format(filtro['$lte'], 'dd-MM-yyyy HH:mm:ss', { locale: es })} /> </>
...
export default function ResumenFiltrosActivos({ filtros }) {

	let Maestros = useContext(ContextoMaestros);
	...
	listaFiltros = clavesDeFiltro.map(clave => <ListItem disablePadding key={clave}>
		<ListItemText primary={textoDeFiltro(filtros, clave, Maestros)} />
	</ListItem>
	)
